...
/**
 * WeakCondorcetWinner Component
 * 
 * Handles display when there's a weak Condorcet winner
 * (candidate who doesn't lose to anyone but may have ties).
 * ~300 lines extracted from the original 1500+ line component.
 */
const WeakCondorcetWinner = ({ 
  results, 
  winner, 
  calculateVotes, 
  getBallotSupport,
  ...
  return (
    <Box>
      ...
      <Box textAlign="center" mb={5}>
        ...
        >
          {winner} is the only candidate that does not lose to any other candidate head-to-head
        </Typography>
      </Box>
      ...
      <Grid container spacing={4} justifyContent="center">
        {candidates
          ...
          .map((opponent) => {
            ...
            const isTied = values.winner === values.opponent;
            ...
            return (
              <Grid item xs={12} md={10} lg={8} key={opponent}>
                <VoteComparison
                  ...
                  colorA={WINNER_TYPE_COLORS.win}  // Weak Condorcet winner always gets green
                  colorB={WINNER_TYPE_COLORS.loss}  // Always red
                  animate={shouldAnimate}
                  ballotSupport={supportData}
                  ballotTypes={ballotTypes}
                  resultText={
                    isTied 
                      ? `Tied with ${values.winner} votes each`
                      : `${winner} wins by ${values.winner - values.opponent} votes`
                  }
                  expanded={isExpanded}
...
export default WeakCondorcetWinner;